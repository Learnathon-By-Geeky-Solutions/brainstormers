...
WORKDIR /source/TaskForge.NET

# Build the application.
# Leverage a cache mount to /root/.nuget/packages so that subsequent builds don't have to re-download packages.
# If TARGETARCH is "amd64", replace it with "x64" - "x64" is .NET's canonical name for this and "amd64" doesn't
#   work in .NET 6.0.
RUN --mount=type=cache,id=s/a5801c42-fe28-41f6-965c-17de80c2b789-/root/.nuget/packages,target=/root/.nuget/packages \
    dotnet publish TaskForge.WebUI/TaskForge.WebUI.csproj -a x64 --use-current-runtime --self-contained false -o /app
...
ENTRYPOINT ["dotnet", "TaskForge.WebUI.dll"]